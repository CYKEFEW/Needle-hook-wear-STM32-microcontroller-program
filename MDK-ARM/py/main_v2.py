...
class SerialPlotApp:
    ...
    def build_ui(self):
        ...
        self.port_combo = ttk.Combobox(
            top, textvariable=self.port_var, font=self.font_big, width=10)
        self.port_combo.pack(side="left", padx=10)
        self.refresh_ports()

        ttk.Button(top, text="刷新端口", command=self.refresh_ports, width=20).pack(side="left", padx=10)
        ttk.Button(top, text="连接", command=self.connect_port, width=20).pack(side="left", padx=10)
        ttk.Button(top, text="断开", command=self.disconnect_port, width=20).pack(side="left", padx=10)

        ttk.Button(top, text="开始采集", command=self.start, width=20).pack(side="left", padx=10)
        ttk.Button(top, text="停止采集", command=self.stop, width=20).pack(side="left", padx=10)
        ttk.Button(top, text="保存CSV", command=self.save_csv, width=20).pack(side="left", padx=10)
        ...
        ttk.Button(send_frame, text="发送", command=self.send_data, width=6).pack(side="left")
                    ...
                    if "}" in buffer:
                        while "{" in buffer and "}" in buffer:
                            ...
                            if "voltage:" in packet:
                                try:
                                    v = float(packet.split(":")[1])
                                    self.data.append(v)
                                    self.timestamps.append(datetime.now())

                                    if len(self.data) > self.max_window:
                                        self.data = self.data[-self.max_window:]
    ...
    def update_plot(self):
        if len(self.data) > 0:
            show_y = self.data[-self.window_size:]
            self.line.set_data(range(len(show_y)), show_y)

            self.ax.set_xlim(0, len(show_y))
            self.ax.set_ylim(min(show_y) - 0.1, max(show_y) + 0.1)
    ...
    def save_csv(self):
        ...
        with open(file, "w", newline="", encoding="utf-8") as f:
            ...
            for t, v in zip(self.timestamps, self.data):
                writer.writerow([t.strftime("%Y-%m-%d %H:%M:%S.%f"), v])
